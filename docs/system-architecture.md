...
- `lab_submission (id, user_id, module_id, lab_id, content, passed, check_results jsonb, created_at)` — every graded-lab submission attempt kept (history), indexed on `(user_id, module_id, lab_id)`. Only labs with a `submission` spec (`LabDefinition.submission`, pilot: `m04-docker-containers`) use this table; other labs keep the plain `progress_item` self-tick.
